...
D3DXVECTOR2 CSprite::GetDimension() const
{
	D3DXVECTOR2 dimension(0, 0);

	// Get the current frame to use
	const CTexture* pTexture = GetTexture();
	if(pTexture)
	{
		dimension.x = static_cast<int>(pTexture->GetWidth() * mScale.x);
		dimension.y = static_cast<int>(pTexture->GetHeight() * mScale.y);
	}
	return dimension;
}
...
float CSprite::GetDimensionX() const
{
	// Get the current frame to use
	const CTexture* pTexture = GetTexture();
	if(pTexture)
	{
		return static_cast<int>(pTexture->GetWidth() * mScale.x);
	}
	return 0;
}
...
float CSprite::GetDimensionY() const
{
	// Get the current frame to use
	const CTexture* pTexture = GetTexture();
	if(pTexture)
	{
		return static_cast<int>(pTexture->GetHeight() * mScale.y);
	}
	return 0;
}
...
// Remember: Every time the scale is changed, the origin of the sprite must be changed as well
...
{
	D3DXVECTOR2 scale(mScale.x, mScale.y);
...
void CSprite::SetPosition(D3DXVECTOR2 position)
{
...
void CSprite::SetPositionX(float fPosX)
{
	mPosition.x = fPosX;
}
...
void CSprite::SetPositionY(float fPosY)
{
	mPosition.y = fPosY;
}
...
void CSprite::SetScale(D3DXVECTOR2 scale)
{
	mScale.x = scale.x;
	mScale.y = scale.y;
}
...
void CSprite::SetScaleX(float fScaleX)
{
	mScale.x = fScaleX;
}
...
void CSprite::SetScaleY(float fScaleY)
{
	mScale.y = fScaleY;
}
